...
public class Marketing extends User {
    ...
    public Report generateOffersReport(String reportTitle) {
        ...
        for (Offer offer : offers) {
            if (offer.isActive())
                ...
                expired++;
            if (offer.getDiscount() > maxDiscount)
                maxDiscount = offer.getDiscount();
        }
    ...
    public Report saveCustomReport(int id, String title, String content) {
        ...
        Report r = new Report(id, title, totalOffers, activeOffers, expiredOffers, maxDiscountValue, new Date(),
                content);
